...
drgui_element* drgui_create_scrollbar(dred_context* pDred, drgui_element* pParent, dred_scrollbar_orientation orientation, size_t extraDataSize, const void* pExtraData)
{
    ...
    drgui_scrollbar* pSB = (drgui_scrollbar*)drgui_get_extra_data(pScrollbar);
    assert(pSB != NULL);
...
void dred_scrollbar_on_paint(dred_scrollbar* pScrollbar, drgui_rect relativeClippingRect, void* pPaintData)
{
    ...
    const drgui_scrollbar* pSB = (const drgui_scrollbar*)drgui_get_extra_data(pScrollbar);
    if (pSB == NULL) {
        return;
    }
