...
dred_scrollbar* dred_scrollbar_create(dred_context* pDred, dred_element* pParent, dred_scrollbar_orientation orientation, size_t extraDataSize, const void* pExtraData)
{
    ...
    dred_scrollbar* pScrollbar = drgui_create_element(pDred, pParent, DRED_CONTROL_TYPE_SCROLLBAR, sizeof(dred_scrollbar_data) + extraDataSize);
    if (pScrollbar == NULL) {
        return NULL;
    }

    dred_scrollbar_data* pSB = (dred_scrollbar_data*)drgui_get_extra_data(pScrollbar);
    assert(pSB != NULL);
...
void dred_scrollbar_on_paint(dred_scrollbar* pScrollbar, dred_rect relativeClippingRect, void* pPaintData)
{
    ...
    const dred_scrollbar_data* pSB = (const dred_scrollbar_data*)drgui_get_extra_data(pScrollbar);
    if (pSB == NULL) {
        return;
    }
