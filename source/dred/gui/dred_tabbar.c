...
DRED_GUI_PRIVATE void dred_tabbar_on_measure_tab_default(dred_tabbar* pTabBar, dred_tab* pTab, float* pWidthOut, float* pHeightOut)
{
    ...
    if (pTab != NULL) {
        dred_gui_measure_string(pTabBar->pFont, pTab->text, strlen(pTab->text), &textWidth, &textHeight);
    }
...
void dred_tab_set_text(dred_tab* pTab, const char* text)
{
    ...
    if (dred_gui_is_auto_dirty_enabled(dred_control_get_gui(DRED_CONTROL(pTab->pTabBar)))) {
...
dred_tabgroup* dred_tab_get_tabgroup(dred_tab* pTab)
{
    ...
    return DRED_TABGROUP(dred_control_get_parent(DRED_CONTROL(pTab->pTabBar)));
}
