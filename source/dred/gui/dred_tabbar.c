...
{
    dred_tabbar* pTabBar = drgui_create_element(pDred, pParent, DRED_CONTROL_TYPE_TABBAR, sizeof(dred_tabbar_data) + extraDataSize);
    if (pTabBar == NULL) {
        return NULL;
    }

    dred_tabbar_data* pTB = (dred_tabbar_data*)drgui_get_extra_data(pTabBar);
    assert(pTB != NULL);
