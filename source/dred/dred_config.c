...
void dred_config_load_file__on_pair(void* pUserData, const char* key, const char* value)
{
    ...
    dred_config_set__autogenerated(pData->pConfig, key, value, 0);
}
...
void dred_config_set(dred_config* pConfig, const char* name, const char* value, dtk_uint32 flags)
{
    ...
    dred_config_set__autogenerated(pConfig, name, value, flags);
}

void dred_config_set_int(dred_config* pConfig, const char* name, dtk_int64 value, dtk_uint32 flags)
{
    char valueStr[128];
    if (dtk_int64_to_string(value, valueStr, sizeof(valueStr), 10) == 0) {
...
{
    char valueStr[128];
    if (dtk_uint64_to_string(value, valueStr, sizeof(valueStr), 10) == 0) {
        dred_config_set(pConfig, name, valueStr, flags);
    }
}
...
{
    // TODO: A public domain double-to-string conversion API is required...
    char valueStr[128];
    int result = snprintf(valueStr, sizeof(valueStr), "%f", value);
    if (result > 0 && result < sizeof(valueStr)) {
        dred_config_set(pConfig, name, valueStr, flags);
    }
}
...
{
    dred_config_set(pConfig, name, (value) ? "true" : "false", flags);
}
...
void dred_config_set_default(dred_config * pConfig, const char * name)
{
    ...
    dred_config_set_default__autogenerated(pConfig, name, 0);
}
