...
FILE* dred__open_log_file(dred_context* pDred)
{
    ...
    FILE* pFile;
    dtk_result result = dtk_fopen(&pFile, logFilePath, dtk_fopenmode(DTK_OPEN_MODE_WRITE));
...
void dred_log(dred_context* pDred, const char* message)
{
    ...
    if (pDred->logFile != NULL) {
        ...
        dtk_fwrite_string(pDred->logFile, "[");
        dtk_fwrite_string(pDred->logFile, dateTime);
        dtk_fwrite_string(pDred->logFile, "]");
        dtk_fwrite_line  (pDred->logFile, message);
        dtk_fflush(pDred->logFile);
    }
...
void dred_save_dredprivate(dred_context* pDred)
{
    ...
    FILE* pFile;
    dtk_result result = dtk_fopen(&pFile, dredprivatePath, dtk_fopenmode(DTK_OPEN_MODE_WRITE));
    if (result != DTK_SUCCESS) {
        return;
    }

    dtk_fwrite_string(pFile, "# This file is generated by dred - do not modify. This file also includes information you may want\n");
    dtk_fwrite_string(pFile, "# to keep private such as paths to your most recent files, so you should not distribute this file.\n\n");

    dred_config_write_to_file__autogenerated(&pDred->config, pFile);
    ...
    for (size_t i = 0; i < pDred->shortcutTable.count; ++i) {
        ...
        dtk_fwrite_string(pFile, bindingStr);
    }

    // Recent files.
    for (size_t i = pDred->config.recentFileCount; i > 0; --i) {
        dtk_fwrite_string(pFile, "recent-file \"");
        dtk_fwrite_string(pFile, pDred->config.recentFiles[i-1]);
        dtk_fwrite_string(pFile, "\"\n");
    }

    // Favourite files.
    for (size_t i = 0; i < pDred->config.favouriteFileCount; ++i) {
        dtk_fwrite_string(pFile, "favourite-file \"");
        dtk_fwrite_string(pFile, pDred->config.favouriteFiles[i]);
        dtk_fwrite_string(pFile, "\"\n");
    }

    // Recent commands.
    for (size_t i = pDred->config.recentCommandsCount; i > 0; --i) {
        dtk_fwrite_string(pFile, "recent-cmd \"");
        dtk_fwrite_string(pFile, pDred->config.recentCommands[i-1]);
        dtk_fwrite_string(pFile, "\"\n");
    }

    // Aliases
    for (size_t i = 0; i < pDred->config.aliasMap.count; ++i) {
        dtk_fwrite_string(pFile, "alias ");
        dtk_fwrite_string(pFile, pDred->config.aliasMap.keys[i]);

        dtk_fwrite_string(pFile, " \"");
        dtk_fwrite_string(pFile, pDred->config.aliasMap.values[i]);
        dtk_fwrite_string(pFile, "\"\n");
    }
