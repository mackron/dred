...
dtk_bool32 dred_image_library_init__stock_images(dred_image_library* pLibrary)
{
    ...
    dtk_uint32 runningID = 0;

    // SVG images.
    for (dtk_uint32 i = 0; i < DRED_STOCK_IMAGE_COUNT_SVG; ++i) {
        if (dtk_image_init_svg(&pLibrary->pDred->tk, g_StockImagesSVG[i].pSVGData, &pLibrary->pStockImages[runningID]) != DTK_SUCCESS) {
            dtk_zero_object(&pLibrary->pStockImages[runningID]);    // Failed to initialize the image. Zero it out. TODO: Replace this with a default fallback.
        }
    ...
    for (dtk_uint32 i = 0; i < DRED_STOCK_IMAGE_COUNT_RASTER; ++i) {
        if (dtk_image_init_raster(&pLibrary->pDred->tk, g_StockImagesRaster[i].width, g_StockImagesRaster[i].height, g_StockImagesRaster[i].width*4, g_StockImagesRaster[i].pImageData, &pLibrary->pStockImages[runningID]) != DTK_SUCCESS) {
            dtk_zero_object(&pLibrary->pStockImages[runningID]);    // Failed to initialize the image. Zero it out. TODO: Replace this with a default fallback.
        }
...
dtk_image* dred_image_library_get_image_by_id(dred_image_library* pLibrary, unsigned int id)
{
    ...
    // Optimization for stock images.
    if (id < DRED_STOCK_IMAGE_COUNT) {
        return &pLibrary->pStockImages[id];
    }
