...
void dred_build__generate_menus(FILE* pFileOut, FILE* pFileOutH, dred_string_pool* pStringPool)
{
    ...
    fprintf(pFileOutH, "#define DRED_MENU_ITEM_ID_FAVOURITE_FILE_0 %u\n", favouriteFile0ID);

    dtk_uint32 theme0ID = favouriteFile0ID + DRED_MAX_FAVOURITE_FILES;
    fprintf(pFileOutH, "#define DRED_MENU_ITEM_ID_THEME_0 %u\n", theme0ID);
    ...
    fwrite_string(pFileOut, "void dred_init_stock_menus__autogenerated(dred_context* pDred)\n{\n");
    {
        ...
        fwrite_string(pFileOut, "    char shortcutStr[256];\n");
        fwrite_string(pFileOut, "    size_t shortcutIndex;\n");
        ...
        for (int iMenu = 0; iMenu < stb_sb_count(context.pAllMenus); ++iMenu) {
            ...
            for (int iItem = 0; iItem < stb_sb_count(pMenu->pItemsIndices); ++iItem) {
                ...
                if (pItem->shortcut[0] != '\0') {
                    char shortcutIDStr[256];
                    dred_build__format_shortcut_id_macro(pItem->shortcut, shortcutIDStr);

                    fprintf(pFileOut, "        if (dred_shortcut_table_find(&pDred->shortcutTable, %s, &shortcutIndex)) {\n", shortcutIDStr);
                    fprintf(pFileOut, "            dred_shortcut_to_string(pDred->shortcutTable.pShortcuts[shortcutIndex], shortcutStr, sizeof(shortcutStr));\n");
                    fprintf(pFileOut, "            item.shortcut = shortcutStr;\n");
                    fprintf(pFileOut, "        }\n");
                }
    ...
    fwrite_string(pFileOut, "void dred_menu_item_table__bind_stock_items__autogenerated(dred_context* pDred, dred_menu_item_table* pTable)\n{\n");
    {
        fprintf(pFileOut, "    (void)pDred;\n\n");

        fprintf(pFileOut, "    pTable->pItems[DRED_MENU_ITEM_ID_NONE].id = DRED_MENU_ITEM_ID_NONE;\n");
        fprintf(pFileOut, "    pTable->pItems[DRED_MENU_ITEM_ID_NONE].commandStrOffset = 0;\n");
        fprintf(pFileOut, "    pTable->pItems[DRED_MENU_ITEM_ID_NONE].shortcutStrOffset = 0;\n\n");

        fprintf(pFileOut, "    pTable->pItems[DRED_MENU_ITEM_ID_SEPARATOR].id = DRED_MENU_ITEM_ID_SEPARATOR;\n");
        fprintf(pFileOut, "    pTable->pItems[DRED_MENU_ITEM_ID_SEPARATOR].commandStrOffset = 0;\n");
        fprintf(pFileOut, "    pTable->pItems[DRED_MENU_ITEM_ID_SEPARATOR].shortcutStrOffset = 0;\n\n");

        for (int iItem = 0; iItem < stb_sb_count(context.pAllItems); ++iItem) {
            dred_build__menu_item* pItem = &context.pAllItems[iItem];

            if (strcmp(context.pAllItems[iItem].id, "separator") != 0 && strcmp(context.pAllItems[iItem].id, "none") != 0) {
                char idMacroStr[256];
                ...
                if (iItem != stb_sb_count(context.pAllItems)-1) {
                    fprintf(pFileOut, "\n");
                }
            }
        }
