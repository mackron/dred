...
void dred_build__generate_shortcuts(FILE* pFileOut, FILE* pFileOutH, dred_string_pool* pStringPool)
{
    ...
    fwrite_string(pFileOut, "void dred_init_stock_shortcuts__autogenerated(dred_context* pDred)\n{\n");
    {
        for (int i = 0; i < stb_sb_count(context.pShortcuts); ++i) {
            ...
            char shortcutInitStr[256];
            dred_build__format_shortcut_init_string(shortcutInitStr, sizeof(shortcutInitStr), context.pShortcuts[i].key);

            fprintf(pFileOut, "    dred_bind_shortcut(pDred, %s, %s, \"%s\");", idStr, shortcutInitStr, cmdStr);
