...
dr2d_context* dr2d_create_context(dr2d_drawing_callbacks drawingCallbacks, size_t contextExtraBytes, size_t surfaceExtraBytes, size_t fontExtraBytes, size_t imageExtraBytes, const void* pUserData)
{
    ...
    if (pContext != NULL)
    {
        ...
        // we want to call the create_context callback and check it's return value. If it's return value if DR_FALSE it means there
...
dr2d_surface* dr2d_create_surface(dr2d_context* pContext, float width, float height)
{
    if (pContext != NULL)
    {
        ...
        if (pSurface != NULL)
        {
            ...
            // we want to call the create_surface callback and check it's return value. If it's return value if DR_FALSE it means there
            // was an error and we need to return null.
...
void dr2d_draw_image_gdi(dr2d_surface* pSurface, dr2d_image* pImage, dr2d_draw_image_args* pArgs)
{
    ...
    {
        assert(drawFlipped == DR_FALSE);   // <-- Error if this is hit.
...
dr_bool32 dr2d_get_glyph_metrics_cairo(dr2d_font* pFont, unsigned int utf32, dr2d_glyph_metrics* pGlyphMetrics)
{
    ...
    if (utf8len == 0) {
        return DR_FALSE;   // Error converting UTF-32 to UTF-8.
    }
